...
class KNNService {
  ...
  static KNNRecommendation? recommendExercise(
    ...
    for (int i = 0; i < neighborPerformances.length; i++) {
      ...
      setsSum += (performance.totalSets ~/ 3) * similarity.round(); // Average sets per session
    }
  ...
  static PerformanceComparison comparePerformance(
    ...
    // Get current 1RM
    final current1RM = UserExerciseHistory.calculateOneRepMax(currentWeight, currentReps);
    ...
    if (difference > 10) {
      ...
      message = '📈 أنت أقل من المتوسط بـ ${difference.abs()}%. حاول زيادة الوزن تدريجياً';
    } else {
  ...
  static WeightSuggestion suggestNextWeight(
    ...
    final avgIncrease = progressions.reduce((a, b) => a + b) / progressions.length;
    final roundedIncrease = (avgIncrease / 2.5).round() * 2.5; // Round to nearest 2.5
    
    return WeightSuggestion(
      suggestedWeight: currentWeight + roundedIncrease.clamp(2.5, 10.0),
      confidence: 0.7,
      reason: 'المستخدمون المشابهون زادوا بمتوسط ${avgIncrease.toStringAsFixed(1)} كجم',
    );
  }
  ...
  static KNNRecommendation _createDefaultRecommendation(
    ...
    return KNNRecommendation(
      ...
      recommendedSets: 3,
      confidence: 0.3,
      similarUsers: neighbors.take(2).map((n) => SimilarUser(
        userId: n.user.id,
        name: n.user.name,
        similarityScore: n.similarityScore,
        theirWeight: 0,
      ...
      reason: 'توصية أولية بناءً على مستواك (${target.level.displayName}) وهدفك',
    );
  }
