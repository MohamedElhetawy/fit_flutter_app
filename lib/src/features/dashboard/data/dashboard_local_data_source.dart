...
class DashboardLocalDataSource {
  ...
  void updateStepsAndCaloriesSync(
      String dateStr, int steps, int activeCalories) {
    _isar.writeTxnSync(() {
      final existing = _isar.dailyStatsLocals
              .filter()
              .dateEqualTo(dateStr)
              .findFirstSync() ??
          DailyStatsLocal()
      ...
      if (activeCalories > existing.activeCalories) {
  ...
  Future<int> addHydration(String dateStr, int hydrationMl) async {
    ...
    await _isar.writeTxn(() async {
      final existing = await _isar.dailyStatsLocals
              .filter()
              .dateEqualTo(dateStr)
              .findFirst() ??
          DailyStatsLocal()
  ...
  Stream<List<DailyStatsLocal>> watchHistoricalStats(int daysPrevious) {
    ...
    final thresholdStr =
        "${thresholdDate.year}-${thresholdDate.month.toString().padLeft(2, '0')}-${thresholdDate.day.toString().padLeft(2, '0')}";
